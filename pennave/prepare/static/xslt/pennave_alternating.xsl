  ...
  <xsl:template match="pa:photoView">
    ...
    <html>
      <head>
        ...
        <script language="javascript">
          var prefix="<xsl:value-of select="/*/@prefix"/>";
          var imgprefixA="<xsl:value-of select="/*/@imgprefixA"/>";
          var imgprefixB="<xsl:value-of select="/*/@imgprefixB"/>";
            ...
            <td class="mainPhoto" align="center" valign="top">
              ...
              <span class="mainPhotoCell" style="padding-top: {$topPadding}px; padding-bottom: {$topPadding}px;">
                <a alt="{/pa:photoView/pa:photo/@name}" title="Klicken, um das Photo in Originalgroesse anzuzeigen."
                 href="{/*/@imgprefixA}/ID-{$idFormat}_Version-{$versionFormat}_Size-original"
                 ><img src="{/*/@imgprefixB}/ID-{$idFormat}_Version-{$versionFormat}_Size-medium" 
  ...
  <xsl:template match="pa:photo">
    ...
    <xsl:choose>
      <xsl:when test="/pa:photoView">
        ...
        <div class="photoThumbnail" id="photoThumbnail{@id}">
          <a href="{/*/@prefix}/view/{@id}/{normalize-space($tagLinks)}"><img 
            src="{/*/@imgprefixA}/ID-{$idFormat}_Version-01_Size-thumbnail"
      ...
      <xsl:otherwise>
        ...
        <!-- Es soll so sein, das die Bilder abwechselnd von http://s3.amazonaws.com/$BUCKET und
            http://$BUCKET.s3.amazonaws.com geladen werden. Dazu gibt es die "Variablen" imprefixA
            und imgprefixB. -->

        <xsl:variable name="imgprefix"><xsl:choose><xsl:when test="(position() mod 2) = 0"><xsl:value-of select="/*/@imgprefixA"/></xsl:when><xsl:otherwise><xsl:value-of select="/*/@imgprefixB"/></xsl:otherwise></xsl:choose></xsl:variable>

...
#import pickle
import cPickle
# pickle.load(urllib.urlopen(url))
...
class ExifTool:
    ...
    def getS3(self, pictureId = None, versionId = None):
        ...
        url = "http://%s.s3.amazonaws.com/%s/ID-%06d_Version-%02d_Size-exif" % (
          cherrypy.config["aws_s3_bucket"], cherrypy.config["aws_s3_path"],
          pictureId, versionId)
        
        log.info(str(pictureId) + "," + str(versionId) + ": EXIF Daten noch nicht im Cache vorhanden. Von %s downloaden." % url)
    ...
    def get(self, filename):
        ...
        print "tags.keys in ExifTool get: %s" % tags.keys()
        x=tags.keys()
        x.sort()
        for i in x:
            if i in ('JPEGThumbnail', 'TIFFThumbnail'):

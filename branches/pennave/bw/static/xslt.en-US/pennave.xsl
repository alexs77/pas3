...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:variable name="tagPrefix">tags<xsl:for-each select="/pa:photoView/pa:rootTags/pa:tag">/<xsl:value-of select="@id"/></xsl:for-each>/</xsl:variable>
          ...
          <div class="sidebar">
            ...
            <xsl:if test="pa:photos/@perPage &lt; pa:photos/@numPhotos">
              ...
              <div class="pageFlicker">
                ...
                Photos <xsl:value-of select="pa:photos/@startPhoto"/> to <xsl:value-of select="pa:photos/@stopPhoto"/> of <xsl:value-of select="pa:photos/@numPhotos"/>
            ...
            <div class="photoInfo" id="photoInfo">
              <span id="photoInfoFilename" class="photoInfoField">Filename: <span class="photoInfoData" id="photoInfoFilenameData"></span></span>
              <span id="photoInfoSize" class="photoInfoField">Size: <span id="photoInfoWidthData" class="photoInfoData"></span> x <span id="photoInfoHeightData" class="photoInfoData"></span></span>
              <span id="photoInfoTags" class="photoInfoField">Tags: <span id="photoInfoTagsData" class="photoInfoData"/></span>
              <span id="photoInfoDescription" class="photoInfoField">Description: <span id="photoInfoDescriptionData" class="photoInfoData"/></span>
              <span id="photoInfoDate" class="photoInfoField">Date: <span id="photoInfoDateData" class="photoInfoData"/></span>
            </div>
          </div>
          <div class="main">
  ...
  <xsl:template match="pa:photoView">
    ...
    <html>
      <head>
        <title>PennAve Viewing Photo</title>
        <link rel="stylesheet" type="text/css" href="{/*/@prefix}/css/pennave.css"/>
        <script type="text/javascript" src="{/*/@prefix}/js/tagSelections.js"/>
        <script language="javascript">
          var prefix="<xsl:value-of select="/*/@prefix"/>";
          ...
          <tr>
            ...
            <td class="photoFlicker" valign="top">
              <xsl:choose>
                ...
                <xsl:otherwise>
                  <div class="photoThumbnail" style="float: none;">You are on the first photo</div>
                </xsl:otherwise>
              </xsl:choose>
              <xsl:choose>
                ...
                <xsl:otherwise>
                  <div class="photoThumbnail">You are on the last photo</div>
                </xsl:otherwise>
              </xsl:choose>
              ...
              <div>
                <span class="photoFlickerTags">
                  ...
                  <xsl:variable name="urlPrefix"><xsl:value-of select="$viewPrefix"/><xsl:value-of select="$tagPrefix"/></xsl:variable>
                    ...
                    Additional Tags:
                    <ul>
                      <xsl:for-each select="/pa:photoView/pa:photo/pa:tags/pa:tag"><li><a href="{$urlPrefix}{@id}/{$versionPrefix}" title="Add &quot;{.}&quot; as an additional filter tag"><xsl:value-of select="."/></a></li></xsl:for-each>
                    </ul>
                </span>
              </div>
              </xsl:if>
              ...
              <div>
                <span class="viewOptions"> 
                  Viewing Options:
                  <ul>
                    <xsl:if test="count(/pa:photoView/pa:rootTags/pa:tag) &gt; 0"><li><a href="{@prefix}/view/{/pa:photoView/pa:photo/@id}" title="Remove all filter tags">Unselect all tags</a></li></xsl:if>
                    <li><a href="javascript:launchSlideShow();">Launch Slide Show</a></li>
                    ...
                    <li><a href="{$browseURL}" title="Go back to the page with many pictures for faster browsing">Return to Browse Mode</a></li>
                  </ul>
                </span>
              </div>
              <div>
                <span class="exifTags">
                  <xsl:choose>
                    <xsl:when test="/pa:photoView/pa:photo/exif:IFD/exif:model">Taken with a <xsl:value-of select="/pa:photoView/pa:photo/exif:IFD/exif:model"/><xsl:if test="/pa:photoView/pa:photo/@asctime"> on <xsl:value-of select="/pa:photoView/pa:photo/@asctime"/></xsl:if>.<br/></xsl:when>
                    <xsl:when test="/pa:photoView/pa:photo/@asctime">Taken on <xsl:value-of select="/pa:photoView/pa:photo/@asctime"/>.<br/></xsl:when>
                  </xsl:choose>
                </span>
              </div>
            </td>
  ...
  <xsl:template match="pa:tagCloud">
    <div class="tagCloud" name="tagCloud">
      <span class="tagCloudTitle">Tags</span>
      <span class="tagCloudTags">
        <xsl:apply-templates select="pa:tag"/>
      </span>
      <xsl:if test="count(//pa:tagCloud/pa:tag) &gt; 30"><span class="tagCloudToggle"><a href="javascript:showAllTagsToggle('Show All {count(//pa:tagCloud/pa:tag)} Tags', 'Show Top 30 Tags');" id="showAllTags">Show All <xsl:value-of select="count(//pa:tagCloud/pa:tag)"/> Tags</a></span></xsl:if>
      <xsl:if test="count(/pa:tagBrowse/pa:rootTags/pa:tag) &gt; 0"><span class="tagCloudToggle"><a href="{/pa:tagBrowse/@prefix}/tags/">Unselect all tags</a></span></xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template match="pa:rootTags">
    <div class="rootTags" id="rootTags">
      <xsl:if test="count(pa:tag) &gt; 0">
        Viewing Tags: <xsl:apply-templates/>
      </xsl:if>
      <xsl:if test="count(pa:tag) = 0">&#160;</xsl:if>
      <div class="pennAveFooter">
        Powered by <a href="http://pennave.sf.net/" title="Visit the PennAve Home Page">PennAve</a>
      </div>
  ...
  <xsl:template match="pa:rootTags/pa:tag">
    ...
    <a href="{$viewOnlyTagURL}" title="show only the tag &quot;{.}&quot;"><xsl:value-of select="."/></a>&#160;<span class="removeTagLink">[<a href="{$removeTagURL}" class="removeTagLink" title="remove the tag &quot;{.}&quot;">-</a>]</span><xsl:if test="following-sibling::pa:tag">&#160;&#x2022;&#160;</xsl:if>
  </xsl:template>
